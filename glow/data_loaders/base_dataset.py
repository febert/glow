...
class BaseVideoDataset:
    ...
    def _get_default_hparams(self):
        default_dict = {'shuffle': True,
                        ...
                        'buffer_size': 512,
                        'compressed': True,
                        'pad':True,   #pad image height to 64
    ...
    def _parse_record(self, serialized_example):
        ...
        def decode_feat(feat, manifest_entry, pad_t=False):
            ...
            if dtype == 'Byte':
                ...
                if self._hparams.pad:
                    zero_pad = tf.zeros([1, 64 - image.get_shape().as_list()[1], 64, 3], dtype=tf.uint8)
                    image = tf.concat([image, zero_pad], axis=1)
                return image
